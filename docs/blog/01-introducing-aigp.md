...
### 1. **Core Protocol** (`@aigp/protocol`)
The JSON schema and TypeScript types. This is the standard.

### 2. **9 Built-in Diagram Types** (`@aigp/plugins`)
- Flowcharts
- Sequence diagrams
- Class diagrams (UML)
- ER diagrams
...
### 3. **7 Layout Engines** (`@aigp/layout`)
...
### 5. **Format Converters** (`@aigp/converters`)
- Mermaid → AIGP
- PlantUML → AIGP
- AIGP → Mermaid
...
```typescript
import { generateArchitectureDiagram } from '@aigp/generators';
...
```typescript
import { analyzePerformance, optimizeDiagram } from '@aigp/converters';
...
```typescript
import type { DiagramPlugin } from '@aigp/protocol';
